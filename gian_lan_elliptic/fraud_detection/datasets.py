...
class EllipticDataset:
    ...
    def train_test_split(self, split_time_step=34):
        """
        Temporal Split: Sử dụng biến time_step (Cột 1) để cắt dữ liệu.
        Chỉ tính trên các nodes đã được gán nhãn (class != 2).
        """
        # Node đã phân loại
        classified_mask = self.merged_df['class'] != 2
        
        # Train (time_step <= 34)
        train_mask = classified_mask & (self.merged_df[1] <= split_time_step)
        train_idx = self.merged_df.index[train_mask].values
        
        # Valid (time_step > 34)
        valid_mask = classified_mask & (self.merged_df[1] > split_time_step)
        valid_idx = self.merged_df.index[valid_mask].values
